-- Restore public.empty_checklist_revision_data and recreate create_checklist_revision
  ...
  SELECT CASE
    WHEN _tipo = 'instalacao'::public.checklist_tipo THEN
      jsonb_build_object(
        ...
        'velocidade', jsonb_build_object('download', '', 'upload', '', 'ping_ms', ''),
        'observacoes', '',
        'assinatura_cliente', null
      )
    ELSE
      jsonb_build_object(
        'sintoma', jsonb_build_object(
          'ont_nao_liga', false,
          'ont_queimada', false,
        ...
        'validacao_fisica', jsonb_build_object(
        ...
        'teste_cabeado', jsonb_build_object(
          'aplicabilidade', null, 'navegacao', false, 'ping', false, 'velocidade', false,
          'cabo_substituido', false, 'download', '', 'upload', '', 'ping_ms', '',
          'funcionou', false, 'apresentou_falha', false, 'ont_reiniciou', false,
          'lan_falhou', false, 'nao_testado', false
        ),
        'teste_wifi', jsonb_build_object(
        ...
        'evidencias_marcadas', jsonb_build_object(
          'etiqueta', false, 'leds', false, 'fonte', false,
          'teste_cabeado', false, 'teste_wifi', false
        ),
        ...
        'noc', jsonb_build_object(
          'autorizada', null, 'analista', '', 'data', '', 'hora', '', 'protocolo', ''
        )
      )
  END
$$;

CREATE OR REPLACE FUNCTION public.create_checklist_revision(_parent_id uuid, _reason text, _stage text, _notes text DEFAULT NULL::text)
